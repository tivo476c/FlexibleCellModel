...
\begin{proposition} \textbf{Edge force} \\
	...
	The edge force $F^{(E)}_j: (\R^2)^{N_V} \rightarrow \R^2$ acting on $\vec{v}_j$ in cell is given by the formula
	\begin{align}
		\begin{split}
			F^{(E)}_j(C) &= - \nabla_{\vec{v}_j} E(C) \\
	...
	\begin{align*}
		...
		&= (E^j_d - E^j_C) (- \frac{1}{2 E^j_C} \begin{pmatrix}
...
\begin{definition} \textbf{Interior angle energy} \\
	...
	\begin{align}
		I(C_i) = \sum\limits_{j=1}^{N_V} \frac{1}{2}| I^{j}_d - I^j_{C_i} |^2, 
	\end{align}
	where $I^{j}_d$ is the desired interior angle at vertex $j$ and $I^j_{C_i}$ is the current interior angle at vertex $j$ of cell $i$. 
\end{definition}
