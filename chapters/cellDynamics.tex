...
Using different values of $k$ allows us to model various types of energies and their corresponding forces, resulting in distinct dynamical behaviours that reflect different aspects of cell physics. \\
...
We define the edge $j$ as $\overline{\vec{v}_j \: \vec{v}_{j+1}}$ and use the operator 
\begin{center}
	$
...
\begin{definition} \textbf{Overlap energy} \\
	...
	\begin{align*}
		&O_k^{i,m}: (\R^{2 N_V})^2 \: \longrightarrow \: \R, \\[0.5em]
		(C_i, C_m) \: \longmapsto \: &O_k^{i,m}(C_i, C_m) = \sum\limits_{D\, \in\, \Omega_{C_i,C_m}} \tfrac{1}{k}|A_{D}|^k.
	\end{align*} 
\end{definition}
...
\begin{proposition} \textbf{Deforming overlap force} \label{force:deformingOverlap}\\
	...
	\begin{align*}
		F_{k}^{\hat{(O)}}(\vec{C}) 
		= - (\nabla_{\vec{v}_1^{\:1}} O_k(\vec{C}), \ldots, \nabla_{\vec{v}_{N_V}^{\:1}} O_k(\vec{C}), \; \cdots \;, \nabla_{\vec{v}_1^{\:N_C}} O_k(\vec{C}), \ldots, \nabla_{\vec{v}_{N_V}^{\:N_C}} O_k(\vec{C}))^T.
	\end{align*}
...
\begin{definition}\textbf{Bounce overlap force} \label{force:bounceOverlap}\\
