...
Therefore, we choose a vertex based cell approach, like in~\cite{Fletcher14, Boromand2018}, in which each cell is represented by a polygon defined by its vertices, enabling the representation of a wide range of cell shapes.
...
First, we want to introduce the DF cell model before establishing the dynamics. 
...
\begin{definition} \textbf{DF SDE} \label{def:DF-Sim}  \\
	The DF SDE considers a system of $N_C \in \N$ cells with $N_V \in \N$ vertices per cell. 
	...
	\end{center}
	We define the vertex wise DF SDE generally as
	\begin{center}
		$ \dequ \vec{v}^{\:i}_j(t) = \F^i_j(\vec{C}(t)) \, \dequ t + \sqrt{2 D} \, \dequ \vec{B}_i(t), \quad 1 \leq i \leq N_{C}, \: 1 \leq j \leq N_{V}$. 
	\end{center}
...
Later on, we also write down a cell wise formulation of the DF SDE in Equation~\eqref{equ:cellwiseDFpde}.
...
Interactions between different cells arise from the overlap forces, which act to resolve overlaps and to prevent cell interpenetration. 
In the process of resolving overlaps, the shape of the cells will change whenever the deforming overlap force is active.  
Once the overlap is resolved, the first three forces act to restore the cell's original shape. \\
In this setup we can investigate the influence of cell deformability to the overall cell diffusion behaviour. 
...
We will use this weak derivative for some of our force computations. 
Next, we introduce the DF energies and dynamics. 
The following energies show a similar structure as the energies in~\cite{Boromand2018}.
...
\begin{proposition}  \textbf{Shoelace formula for DF cells} \label{prop:Shoelace}\\ 
	...
	\begin{center}
		$A_C = \frac{1}{2}\sum\limits_{j = 1}^{N_V} (v_j^{x} v_{j+1}^{y} - v_{j+1}^{x} v_j^{y})$,
	\end{center} 
...
\begin{center}
	...
	$
\end{center}
to denote the length of the edge. 
\begin{definition} \textbf{Edge energy} \\
	The energy $E_k: (\R^2)^{N_V} \rightarrow \R_{+}$, used to keep the edges at a constant length, reads 
	\begin{align}
...
Simulations without the interior angle energy sometimes can also result in constrictions at certain vertices, where the interior angle approaches $360$°. 
...
This function computes the angle of a vector $\vec{v}$ with respect to the positive $x$-axis. \\
We can use it to compute the angles 
\begin{center}
...
However, we just neglect the modulo operator as it does not affect the dynamic of the gradient.
...
\subsection{Overlap forces}
% explain the algorithm
Unlike the previous energies, which act independently on each cell, the overlap forces are the first to account for interactions between multiple cells, thereby introducing cell-to-cell interaction into the DF dynamic. 
To resolve overlaps between cells, two conceptually different interaction forces are introduced. 
The deforming overlap force acts by locally modifying the cell shape, allowing overlaps to be reduced through elastic deformation. 
In contrast, the bounce overlap force resolves overlaps through the collective transport of entire cells, inducing a repulsive motion without altering their shape. 
While both mechanisms serve to eliminate overlaps, they operate on different geometric levels and lead to fundamentally different contributions in the DF dynamics.
...
It degenerates cell overlaps by influencing that cell shapes of the affected cells. 
...
\begin{definition} \textbf{Set of overlaps $\Omega_{C_i,C_m}$}\\
	...
	\[
		\Omega_{C_i,C_m} = \{\, D \mid D \text{ is an overlap cell formed from } C_i \text{ and } C_m \,\}.
	\]
\end{definition}
...
\begin{figure}[h!]
	\begin{center}
		...
		\caption{
			...
			In this example, we have $\text{adj}(\vec{w}_1) = \{\vec{v}_1^{\:1}, \vec{v}_2^{\:1}, \vec{v}_2^{\:2}, \vec{v}_3^{\:2}\}$, $\text{in}(\vec{w}_1) = \{\vec{v}_1^{\:1}, \vec{v}_3^{\:2}\}$ and $\text{out}(\vec{w}_1) = \{\vec{v}_2^{\:1}, \vec{v}_2^{\:2}\}$ for the first intersection.
			For the second intersection, we get $\text{adj}(\vec{w}_2) = \{\vec{v}_1^{\:1}, \vec{v}_6^{\:1}, \vec{v}_4^{\:2}, \vec{v}_5^{\:2}\}$, $\text{in}(\vec{w}_2) = \{\vec{v}_1^{\:1}, \vec{v}_4^{\:2}\}$ and $\text{out}(\vec{w}_2) = \{\vec{v}_6^{\:1}, \vec{v}_5^{\: 2}\}$.
			}
		\label{fig:adjacent}
	\end{center}
\end{figure}
...
\begin{definition} \textbf{Bounce overlap energy} \\
    ...
    \begin{align*}
        ...
        d \: \longmapsto \: &\psi(d) = \tfrac{N_V}{2} \mathbf{1}_{[0, 2r)}(d) (2r - d)^2,
    \end{align*}
...
\begin{proposition} \textbf{Bounce overlap force} \label{prop:bounceForce}\\
    The bounce overlap force $F^{(\bar{O})}_i: (\R^{2N_V})^{N_C} \rightarrow \R^{2N_V}$ that gets applied on cell $i$, is given by
    \begin{align*}
    ...
    \end{align*}
	For the derivative of $\psi$, we first consider $s \in [0,2r]$. 
    On this interval, we have
    \begin{align*}
        \psi'(s) 
        &= \frac{\dequ}{\dequ s} \left[\frac{N_V}{2} \mathbf{1}_{[0, 2r)}(s)  (2r - s)^2\right] \\
        &= \frac{N_V}{2} \frac{\dequ}{\dequ s} \left[(2r - s)^2\right] \\
        &= \frac{N_V}{2} 2(2r - s) \frac{\dequ}{\dequ s} \left[(2r - s)\right] \\
        &= N_V (s - 2r).\\
    \end{align*}
	For $s=2r$, we have a derivative of $\psi'(2r) = 0$. 
	As for $s>2r$, $psi$ is constant zero, we also have $\psi'(s) = 0$ for all $s > 2r$. 
	Overall, we get the continuous derivative 
	\begin{align*}
	    \psi'(s) = N_V \mathbf{1}_{[0, 2r)}(s) (s - 2r).
	\end{align*} 
    Thus, 
    \begin{align*}
    \psi'(\chi(C_i - C_j)) = \psi'(\norm[\vec{x}_i - \vec{x}_j]) = N_V \mathbf{1}_{[0, 2r)}(\norm[\vec{x}_i - \vec{x}_j]) (\norm[\vec{x}_i - \vec{x}_j] - 2r).
    \end{align*}
	...
	For a general function argument $B = (\vec{b}_1, \ldots, \vec{b}_{N_V}) \in \R^{2N_V}$, that we can interpret as a vector of two-dimensional vertices, the problematic division occurs whenever 
	\[
		\sum\limits_{k=1}^{N_V} \vec{b}_k = 0 \in \R^2,
	\]
	thus whenever the centre of mass of that vector of two-dimensional vertices is at the origin. 
...
\]
where $F^{(\bar{O})}$ denotes the bounce overlap force and $F^{\hat{(O)}}$ the shape deforming overlap force.\\
When $h = 1$, the cells are maximally stiff. 
In this case, shape deformation is entirely suppressed: all overlaps are resolved solely through the bounce mechanism, and the shape preserving forces become redundant since the cells always retain their desired configurations. \\
As $h$ decreases, the cells become progressively softer. 
The influence of the bounce overlap force diminishes, while the shape deforming overlap force gains dominance, allowing cells to deform more in response to contact with neighbors. \\
...
For the bounce overlap force, a scaling factor of $10^5$ is necessary to reproduce the original bounce dynamics used in~\cite{Bruna2012}. \\
...
\begin{figure}[h!]
    ...
    \caption{Energy diagram of the hard DF dynamic.
			The bounce overlap force eliminates the overlap within a single time step.
...
\begin{figure}[h!]
    ...
    \caption{Energy diagram of the mid DF dynamic.
			 It shows that the overlap energy drops to zero immediately, while the shape preserving energies increase initially as the cells deform to resolve the overlap.
...
\begin{figure}[t!]
    ...
    \caption{Energy diagram of the soft DF dynamic.
			 The diagram shows that the overlap energy drops initially but then increases again as the cells deform to restore their shapes.
