...
\subsubsection*{Shape deforming overlap force}
...
\begin{proposition} \textbf{Overlap force} \\
	The overlap force $F_{k}^{(O)}: (\R^{2 N_V})^{N_C} \rightarrow (\R^{2 N_V})^{N_C}$ that gets applied on the whole cell system $\vec{C} = (C_1, \ldots, C_{N_C})$ is given by  
	\begin{align*}
		F_{k}^{(O)}(\vec{C}) 
	...
	\end{align*}
	where the gradient $\nabla_{\vec{v}_j} O_k(\vec{C})$ with respect to $\vec{v}_j = (v_{j}^{x}, v_{j}^{y})^T$ is given by 
	\begin{align}
		\begin{split}
			\nabla_{\vec{v}_j} O_k(\vec{C}) &= \sum\limits_{m=1, m \neq i}^{N_C} \left( \sum\limits_{D \in \Omega_{i,m}} \delta_{\vec{v}_j}(D)  \dfrac{1}{2} |A_{D}|^{k-1} \begin{pmatrix} d_{j+1}^{D, 2} - d_{j-1}^{D, 2} \\[0.5em]  d_{j-1}^{D, 1} - d_{j+1}^{D, 1} \end{pmatrix} \right)
		\end{split}
		\label{gradient:overlap}
	\end{align}
	for all $1 \leq j \leq N_V$, where $\Omega_{i,m}$ is the set of all overlaps that arise between the cells $i$ and $m$, $\vec{d}_{j-1}^{\: D} = (d_{j-1}^{D, 1}, d_{j-1}^{D, 2})^T$ and $\vec{d}_{j+1}^{	\: D} = (d_{j+1}^{D, 1}, d_{j+1}^{D, 2})^T$ are the neighboring vertices of $\vec{v}_j$ in the overlap $D$ and $A_{D}$ is the area of the overlap $D$.\\
	The dirac $\delta_{\vec{v}_j}(D)$ equals one, if $\vec{v}_j$ is a vertex of the overlap $D$ and zero otherwise. \\
	For addressing the overlap force that acts on cell $1 \leq i \leq N_C$, we define 
	\begin{center}
		$F_{k, i}^{(O)}: (\R^{2 N_V})^{N_C} \rightarrow (\R^{2 N_V})$, \\
		$F_{k, i}^{(O)}(\vec{C}) = - (\nabla_{\vec{v}_1^{\:i}} O_k(\vec{C}), \ldots, \nabla_{\vec{v}_{N_V}^{\:i}} O_k(\vec{C}))$. 
	\end{center}
	...
	\begin{align*}
		...
		\begin{pmatrix} d_{j+1}^{D, 2} - d_{j-1}^{D, 2} \\[0.5em]  d_{j-1}^{D, 1} - d_{j+1}^{D, 1} \end{pmatrix} \\
	...
	\begin{align*}
		...
		\begin{pmatrix} d_{j+1}^{D, 2} - d_{j-1}^{D, 2} \\[0.5em]  d_{j-1}^{D, 1} - d_{j+1}^{D, 1} \end{pmatrix}\right)\\
	\end{align*}
...
\subsubsection*{Bounce off overlap force} 
...
This additional force, which we refer to as the bounce off overlap force, acts not by changing cell shape but by actively transporting overlapping cells away from each other. 
...
\begin{definition}\textbf{Bounce off overlap force} \\
	...
	% The vector $\dequ \hat{o}_{i,l} \in \R^2$ that gets applied to all vertices of $C_i$, caused by another cell $C_l$ is given by 
	% \[
	% \dequ \hat{o}_{i,l} = \mathbf{1}_{\norm[\vec{x}_i - \vec{x}_l] < 2r} (2r - \norm[\vec{x}_i - \vec{x}_l]) \dfrac{\vec{x}_i - \vec{x}_l}{\norm[\vec{x}_i - \vec{x}_l]}. 
	% \]
	% It is zero if the cell centres have a distance of minimum $2r$. 
	% Otherwise, we move all vertices of $C_i$ away from $C_l$ (direction $\vec{x}_i - \vec{x}_l$) so far, that both cells will have a distance of $2r + a$ after the application of the bounce off overlap force on both cells ($C_j$ gets transported the same distance to the reversed direction $\vec{x}_l - \vec{x}_i$), when $a$ is how much the cells overlapped before ($\norm[\vec{x}_i - \vec{x}_l] = 2r - a$ before force application). \\
	% The force $F^{(\hat{O})}_{i,l}$ that acts on $C_i$ caused by the interaction with $C_l$ is given by
	% \[F^{(\hat{O})}_{i,l}(C_i, C_l) =  (\dequ \hat{o}_{i,l}, ..., \dequ \hat{o}_{i,l})^T \in R^{2 N_V}.\]
	% The total bounce off overlap force $F^{(\hat{O})}_{i}: (\R^{2 N_V})^{N_C} \rightarrow \R^{2 N_V}$ for $C_i$ is then given by 
	% \[
	% 	F^{(\hat{O})}_{i}(\vec{C}) = \sum\limits_{l \neq i} F^{(\hat{O})}_{i,l}(C_i, C_l).
	% \] 
	...
	We define the vector $\dequ \hat{o}_{i,l} \in \mathbb{R}^2$, which is applied equally to all vertices of cell $C_i$, representing the repulsive overlap force caused by cell $C_l$. 
	It is given by
	\[
	\dequ \hat{o}_{i,l} = \mathbf{1}_{\norm[\vec{x}_i - \vec{x}_l] < 2r} (2r - \norm[\vec{x}_i - \vec{x}_l]) \dfrac{\vec{x}_i - \vec{x}_l}{\norm[\vec{x}_i - \vec{x}_l]}.
	\]
	...
	Otherwise, if the cells overlap (i.e., $\norm[\vec{x}_i - \vec{x}_l] < 2r$), the vector $\dequ \hat{o}_{i,l}$ points from $\vec{x}_l$ to $\vec{x}_i$ and has magnitude equal to the overlap depth $a = 2r - \norm[\vec{x}_i - \vec{x}_l]$. 
	...
	This means that all vertices of cell $C_i$ are displaced away from $C_l$ in the direction $\vec{x}_i - \vec{x}_l$ such that the resulting displacement is sufficient to separate the two cells by exactly $2r + a$. 

	The force $F^{(\hat{O})}_{i,l}$ that acts on cell $C_i$ due to its interaction with cell $C_l$ is given by
	\[
	F^{(\hat{O})}_{i,l}(C_i, C_l) =  (\dequ \hat{o}_{i,l}, \ldots, \dequ \hat{o}_{i,l})^T \in \mathbb{R}^{2N_V},
	\]
	where the vector $\dequ \hat{o}_{i,l}$ is repeated $N_V$ times, once for each vertex of cell $C_i$.

	The total bounce off overlap force $F^{(\hat{O})}_i: (\mathbb{R}^{2N_V})^{N_C} \rightarrow \mathbb{R}^{2N_V}$ acting on cell $C_i$ due to all other cells in the system is then defined as
	\[
	F^{(\hat{O})}_i(\vec{C}) = \sum\limits_{l \neq i} F^{(\hat{O})}_{i,l}(C_i, C_l).
	\]
...
%TODO: insert figure of Bounce off overlap force 
...
\[
F^{(\mathbf{O})} = h \cdot F^{(\hat{O})} + (1 - h) \cdot F^{(O)},
\]
where $F^{(\hat{O})}$ denotes the bounce-off overlap force and $F^{(O)}$ the shape-deforming overlap force.\\
