...
\begin{definition} \textbf{Intersection point and adjacent vertices} \\
	...
	\begin{center}
		...
		$w(\vec{v}_{\text{in}}^{\: i}, \vec{v}_{\text{out}}^{\: i}, \vec{v}_{\text{in}}^{\: m}, \vec{v}_{\text{out}}^{\: m}) = \vec{v}_{\text{out}}^{\: i} + \dfrac{(\vec{v}_{\text{out}}^{\: m} - \vec{v}_{\text{out}}^{\: i}) \times (\vec{v}_{\text{in}}^{\: m} - \vec{v}_{\text{out}}^{\: m})}{(\vec{v}_{\text{in}}^{\: i} - \vec{v}_{\text{out}}^{\: i}) \times (\vec{v}_{\text{in}}^{\: m} - \vec{v}_{\text{out}}^{\: m})} (\vec{v}_{\text{in}}^{\: i} - \vec{v}_{\text{out}}^{\: i})$,
	\end{center}
...
\begin{proposition} \textbf{Partial derivatives of intersection points} \\
	We use $w(\vec{v}_{\text{in}}^{\: i}, \vec{v}_{\text{out}}^{\: i}, \vec{v}_{\text{in}}^{\: m}, \vec{v}_{\text{out}}^{\: m})$ to compute the influence of the adjacent vertices to the intersection point via their partial derivatives and the chain rule.
	In this proposition, we compute the needed partial derivatives. 

	\[
		\textbf{Definition.}\quad
		\begin{aligned}
		&\text{Let } \vec{A} := \vec{v}^{\,i}_{\text{out}},\quad 
		\vec{B} := \vec{v}^{\,i}_{\text{in}},\quad 
		\vec{C} := \vec{v}^{\,m}_{\text{out}},\quad 
		\vec{D} := \vec{v}^{\,m}_{\text{in}},\\
		&\text{and set } \vec{r} := \vec{B} - \vec{A},\quad 
		\vec{s} := \vec{D} - \vec{C},\quad
		f := (\vec{C}-\vec{A})\times \vec{s},\quad 
		g := \vec{r} \times \vec{s}.
		\end{aligned}
		\]

		We write the 2D cross product as a scalar via the skew matrix
		\[
		J := \begin{pmatrix}0 & 1 \\ -1 & 0\end{pmatrix}, 
		\qquad \text{so that } a\times b = a^\top J b.
		\]

		The intersection can be written as
		\[
		w(\vec{A},\vec{B},\vec{C},\vec{D}) \;=\; \vec{A} \;+\; \frac{f}{g}\, \vec{r}.
		\]

		---

		\[
		\textbf{Differentiation w.r.t. } \vec{A}:\qquad
		\begin{aligned}
		d\vec{r} &= d\vec{B} - d\vec{A} = -\,d\vec{A},\\
		df &= d\big((\vec{C}-\vec{A})^\top J \vec{s}\big) = -\,d\vec{A}^\top J \vec{s},\\
		dg &= d\big(\vec{r}^\top J \vec{s}\big) = d\vec{r}^\top J \vec{s} = -\,d\vec{A}^\top J \vec{s}.
		\end{aligned}
		\]

		Thus
		\[
		dw \;=\; d\vec{A} \;+\; d\!\left(\frac{f}{g}\right) \vec{r} \;+\; \frac{f}{g}\, d\vec{r}.
		\]

		Expanding,
		\[
		dw \;=\; \Big(I - \frac{f}{g}\,I\Big)\, d\vec{A} 
		\;+\; \vec{r}\left(\frac{1}{g}\,df - \frac{f}{g^2}\,dg\right).
		\]

		Since \(df = dg = -\,d\vec{A}^\top J \vec{s}\),
		\[
		\frac{1}{g}\,df - \frac{f}{g^2}\,dg
		= -\left(\frac{1}{g} - \frac{f}{g^2}\right) d\vec{A}^\top J \vec{s}
		= -\,\frac{g - f}{g^2}\; d\vec{A}^\top J \vec{s}.
		\]

		Therefore
		\[
		dw \;=\; \left(1-\frac{f}{g}\right) I\, d\vec{A} 
		\;-\; \frac{g-f}{g^2}\; \vec{r}\, ( J \vec{s} )^\top d\vec{A}.
		\]
		...
		\[
		\boxed{
		\;\;\frac{\partial w}{\partial \vec{v}^{\,i}_{\text{out}}}
		\;=\;
		\left(1-\frac{f}{g}\right) I
		\;-\;
		\frac{g-f}{g^2}\; (\vec{v}^{\,i}_{\text{in}}-\vec{v}^{\,i}_{\text{out}})\, \big(J(\vec{v}^{\,m}_{\text{in}}-\vec{v}^{\,m}_{\text{out}})\big)^\top
		}
		\]

		with
		\[
		g = (\vec{v}^{\,i}_{\text{in}}-\vec{v}^{\,i}_{\text{out}})\times(\vec{v}^{\,m}_{\text{in}}-\vec{v}^{\,m}_{\text{out}}), 
		\qquad
		f = (\vec{v}^{\,m}_{\text{out}}-\vec{v}^{\,i}_{\text{out}})\times(\vec{v}^{\,m}_{\text{in}}-\vec{v}^{\,m}_{\text{out}}).
	\]
\end{proposition}
...
\begin{proposition} \textbf{Deforming overlap force} \\
	...
	\begin{align}
		\begin{split}
			\nabla_{\vec{v}_j^{\: i}} O_k(\vec{C}) = \sum\limits_{D \in \Omega_{i,m}} \dfrac{1}{2} |A_{D}|^{k-1} \Biggl(& \mathbf{1}_{V(D)}(\vec{v}_j^{\: i}) \begin{pmatrix} d_{j+1}^{y} - d_{j-1}^{y} \\[0.5em]  d_{j-1}^{x} - d_{j+1}^{x} \end{pmatrix} +\\
			& \sum\limits_{\vec{w} \in W(D)} \mathbf{1}_{\text{adj}(w)}(\vec{v}_j^{\: i}) \dfrac{\partial  O_k(\vec{C})}{\partial \vec{w}} \dfrac{\partial \vec{w}}{\partial \vec{v}_j^{\: i}} \Biggr),
		\end{split}
	\end{align}
	...
	\begin{align*}
		...
		&= \nabla_{\vec{v}_j^{\: i}} \sum\limits_{m \neq i} \sum\limits_{D \in \Omega_{i,m}} \frac{1}{k}|A_{D}|^k \\
		&= \sum\limits_{m \neq i} \sum\limits_{D \in \Omega_{i,m}} \nabla_{\vec{v}_j^{\: i}} \frac{1}{k}|A_{D}|^k. 
	\end{align*}
	...
	In order to compute the gradient, we need the partial derivative $\dfrac{\partial w(\vec{v}_{\text{in}}^{\: i}, \vec{v}_{\text{out}}^{\: i}, \vec{v}_{\text{in}}^{\: m}, \vec{v}_{\text{out}}^{\: m})}{\partial \vec{v}_{\text{out}}^{\: i}}$.
	...
	\begin{align*}
		\dfrac{\partial w(\vec{v}_{\text{in}}^{\: i}, \vec{v}_{\text{out}}^{\: i}, \vec{v}_{\text{in}}^{\: m}, \vec{v}_{\text{out}}^{\: m})}{\partial \vec{v}_{\text{in}}^{\: i}} &= 
	\end{align*}
