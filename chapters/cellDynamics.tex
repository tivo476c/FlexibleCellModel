...
\begin{proposition} \textbf{Deforming overlap force} \\
	...
	% \end{center}
	Let $\Omega_{i,j}$ be the set of all overlaps between the cells $C_i$ and $C_j$. 
	Each overlap cell $o \in \Omega_{i,j}$ is a list of vertices, that form the overlap just like a DF cell. 
	The vertices in $o$ can be divided into the vertices that are either in $C_i$ in $C_j$, we call that set $V(o)$, and into the vertices that are neither in $C_i$ or $C_j$, named $W(o) = o \setminus V(o)$.
	All overlap vertices in $W(o)$ are intersections between the cells $C_i$ and $C_j$. 
	...
	We call those four vertices adjacent to the intersection $\vec{w}$. 
	...
	For each intersection $\vec{w} \in W(o)$, we call its adjacent vertices $\text{adj}(\vec{w}) = \{\vec{v}_{\text{in}}^{\: i}, \vec{v}_{\text{out}}^{\: i}, \vec{v}_{\text{in}}^{\: j}, \vec{v}_{\text{out}}^{\: j} \}$.  
	Given the four adjacent vertices, we can always compute the intersection with the function:
	\begin{center}
		$w: (\R^2)^4 \rightarrow \R^2$, \\
		$w(\vec{v}_{\text{in}}^{\: i}, \vec{v}_{\text{out}}^{\: i}, \vec{v}_{\text{in}}^{\: j}, \vec{v}_{\text{out}}^{\: j}) = \vec{v}_{\text{out}}^{\: i} + \dfrac{(\vec{v}_{\text{out}}^{\: j} - \vec{v}_{\text{out}}^{\: i}) \times (\vec{v}_{\text{in}}^{\: j} - \vec{v}_{\text{out}}^{\: j})}{(\vec{v}_{\text{in}}^{\: i} - \vec{v}_{\text{out}}^{\: i}) \times (\vec{v}_{\text{in}}^{\: j} - \vec{v}_{\text{out}}^{\: j})} (\vec{v}_{\text{in}}^{\: i} - \vec{v}_{\text{out}}^{\: i})$,
	\end{center}
	where $(a^x, a^y)^T \times (b^x, b^y)^T = a^x b^y - a^y b^x$ denotes the two dimensional cross product. \\
	We use this function to compute the influence of the adjacent vertices to the intersection point via the partial derivatives $\dfrac{\partial w(\vec{v}_{\text{in}}^{\: i}, \vec{v}_{\text{out}}^{\: i}, \vec{v}_{\text{in}}^{\: j}, \vec{v}_{\text{out}}^{\: j})}{\partial \vec{v}_{\text{in}}^{\: i}}$ and $\dfrac{\partial w(\vec{v}_{\text{in}}^{\: i}, \vec{v}_{\text{out}}^{\: i}, \vec{v}_{\text{in}}^{\: j}, \vec{v}_{\text{out}}^{\: j})}{\partial \vec{v}_{\text{out}}^{\: i}}$. 
