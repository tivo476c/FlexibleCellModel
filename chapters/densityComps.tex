...
For the computation, we use the convolution notation $f * g$ for two functions $f,\, g:\, \R^{d} \to \R$, $d \in \N$, defined as
\begin{align*}
    (f * g)(x) = \int f(x - y) g(y) \: \dequ y.
\end{align*}
...
\begin{align*}
    ...
    &= \alpha_{\bar{O}}  \int  \phi(X) \nabla \cdot \left[ \left(\nabla_{X} \left( \psi \circ \chi \right) * \bar{\rho}_t^{\:N_V}\right)\!(X)  \bar{\rho}_t^{\:N_V}(X) \right] \: \dequ X \\
...
\end{align}
where $X \in \R^{2N_V}$ denotes the full cell configuration vector and $\vec{x} \in \R^2$ the cell center position extracted from $X$.
...
\end{align} 
The four gradients are given in the Equations~\eqref{eq:cGradArea}, \eqref{eq:cGradEdge}, \eqref{eq:cGradAngle} and %TODO: add bounce gradient reference.
