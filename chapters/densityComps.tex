...
\begin{align}
    \begin{split}
        ...
        &= - \int \nabla \phi(\vec{x}) \cdot \nabla E(\vec{x}) \: \dequ \mu^{N_C}_t(\vec{x}) \\
        &= - \int \nabla \phi(\vec{x}) \cdot \nabla E(\vec{x}) \: \rho_t^{N_C}(\vec{x}) \: \dequ \vec{x} \\
        &\xrightarrow{N_C \rightarrow \infty} - \int \nabla \phi(\vec{x}) \cdot \nabla E(\vec{x}) \: \bar{\rho}_t(\vec{x}) \: \dequ \vec{x} \\
        &= \int \phi(\vec{x})  \nabla \cdot ( \bar{\rho}_t(\vec{x}) \nabla E(\vec{x})) \: \dequ \vec{x}. \\
    \end{split}
...
% now compute mean field pde with bounce overlap energy 

%%% bounce cell wise gradient   
% We get the same gradient, no matter which $1 \leq k \leq N_V$ we choose.
% Thus, we can write down the cell wise gradient as 
% \begin{align*}  
%     \nabla_{C_i} \chi (C_i - C_j) 
%     = \begin{pmatrix}
%         \nabla_{\vec{v}^{\:i}_1} \\ 
%         \vdots \\ 
%         \nabla_{\vec{v}^{\:i}_{N_V}} 
%     \end{pmatrix} \chi (C_i - C_j) 
%     = \frac{1}{ N_V\norm[\vec{x}_i - \vec{x}_j]} \begin{pmatrix}
%         I_2 \\ 
%         \vdots \\ 
%         I_2
%     \end{pmatrix} (\vec{x}_i - \vec{x}_j) \; \in \R^{2 N_V},
% \end{align*} 
% which is the vertex wise gradient $N_V$-times copied below each other. 
...
Moreover, the interaction kernel
% \[
%     K(z) 
%     = \mathbf{1}_{\{\|z\| < 2r\}}\, (2r - \|z\|)\,\frac{z}{\|z\|},
% \]
is compactly supported in the ball $B_{2r}(0)$, bounded between $0$ and $2r$, and continuous. 
...
\begin{align*}
    ...
    &= - \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \nabla \phi(C_i(t)) \cdot \frac{\alpha_{\bar{O}}}{N_C} \sum\limits_{j = 1}^{N_C}  \nabla_{C_i} \left( \psi \circ \chi \right) (C_i(t) - C_j(t)) \\
...
\begin{align*}
    ...
    &=   \int  \phi(X) \nabla \cdot \left[ \alpha_{\bar{O}} \bar{\rho}_t^{\:N_V}(X) \left(\nabla_{X} \left( \psi \circ \chi \right) * \bar{\rho}_t^{\:N_V}\right)\!(X)   \right] \: \dequ X. \\
\end{align*}
% TODO: there is still confusion with the lower formulation, what is the argument x and what does \psi \circ \chi(x) then mean?
...
\begin{align*}
    E_{\text{hard}}(\vec{C}) =  E_{\bar{O}}(\vec{C}) + \sum\limits_{i=1}^{N_C}  E_A(C_i) + E_E(C_i) + E_I(C_i).
\end{align*}
% we can use the linearity of the gradient to easily write down 
% \begin{align}
%     \nabla_C E_{\text{hard}}(C) = \nabla_C E_A(C) + \nabla_C E_E(C) + \nabla_C E_I(C) + \nabla_C E_{\bar{O}}(\vec{C}).
% \end{align} 
That dynamic is equivalent to the deterministic part of the DF SDE, given in Equation~\eqref{DF-deterministic-force}.
The cell dynamic is given as 
\begin{align*}
    \frac{\partial C_i}{\partial t} 
    &= - \nabla_{C_i} E_{\text{hard}}(\vec{C}) \\
    &= - \nabla_{C_i} E_{\bar{O}}(\vec{C}) - \nabla_{C_i}E_A(C_i) - \nabla_{C_i}E_E(C_i) - \nabla_{C_i} E_I(C_i),
\end{align*}
where the first gradient is given in Equation~\eqref{cellgrad:bounce} and the shape preserving gradients are given in the Equations~\eqref{eq:cGradArea}, \eqref{eq:cGradEdge}, \eqref{eq:cGradAngle}.
Due to linearity of the integral, scalar product

\begin{align*}
    \frac{\dequ}{\dequ t} &\int \phi(X) \: \dequ \mu^{(N_C, N_V)}_t(X) \\
    &=  \frac{\dequ}{\dequ t} \left[ \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \phi(C_i(t)) \right] \\
    &= - \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \nabla \phi(C_i(t)) \cdot \nabla_{C_i} E_{\text{hard}}(\vec{C}(t)) \\
    &= - \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \nabla \phi(C_i(t)) \cdot \left[ \nabla_{C_i} E_{\bar{O}}(\vec{C}) + \nabla_{C_i}E_A(C_i) + \nabla_{C_i}E_E(C_i) + \nabla_{C_i} E_I(C_i) \right] \\
    &= - \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \nabla \phi(C_i(t)) \cdot \left[ \nabla_{C_i} E_{\bar{O}}(\vec{C}) \right] 
    - \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \nabla \phi(C_i(t)) \cdot \left[ \nabla_{C_i}E_A(C_i) \right]  \\
    &\quad - \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \nabla \phi(C_i(t)) \cdot \left[ \nabla_{C_i}E_E(C_i) \right] 
    - \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \nabla \phi(C_i(t)) \cdot \left[ \nabla_{C_i} E_I(C_i) \right] \\
    &\xrightarrow{N_C \rightarrow \infty}   \int  \phi(X) \nabla \cdot \left[ \alpha_{\bar{O}} \bar{\rho}_t^{\:N_V}(X) \left(\nabla_{X} \left( \psi \circ \chi \right) * \bar{\rho}_t^{\:N_V}\right)\!(X)   \right] \: \dequ X \\
    &\quad + \int \phi(X)  \nabla \cdot ( \bar{\rho}_t^{\:N_V}(X) \nabla E_A(X)) \: \dequ X 
     + \int \phi(X)  \nabla \cdot ( \bar{\rho}_t^{\:N_V}(X) \nabla E_E(X)) \: \dequ X \\
    &\quad + \int \phi(X)  \nabla \cdot ( \bar{\rho}_t^{\:N_V}(X) \nabla E_I(X)) \: \dequ X \\
    &= \int \phi(X)  \nabla \cdot \Biggl[ \bar{\rho}_t^{\:N_V}(X) (\alpha_{\bar{O}}\left(\nabla_{X} \left( \psi \circ \chi \right) * \bar{\rho}_t^{\:N_V}\right)\!(X) \\
    &\quad + \nabla E_A(X) + \nabla E_E(X) + \nabla E_I(X)) \Biggr] \: \dequ X
    % &= - \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \nabla \phi(C_i(t)) \cdot \frac{\alpha_{\bar{O}}}{N_C} \sum\limits_{j = 1}^{N_C}  \nabla_{C_i} \left( \psi \circ \chi \right) (C_i(t) - C_j(t)) \\
    % &= - \frac{\alpha_{\bar{O}}}{{N_C}^2} \sum\limits_{i=1}^{N_C}\sum\limits_{j = 1}^{N_C}  \nabla \phi(C_i(t)) \cdot  \nabla_{C_i} \left( \psi \circ \chi \right) (C_i(t) - C_j(t)) \\
    % &= - \frac{\alpha_{\bar{O}}}{{N_C}}  \int \sum\limits_{j = 1}^{N_C}  \nabla \phi(X) \cdot  \nabla_{X} \left( \psi \circ \chi \right) (X - C_j(t)) \: \dequ \mu^{(N_C, N_V)}_t(X)\\
    % &= - \alpha_{\bar{O}}  \iint  \nabla \phi(X) \cdot  \nabla_{X} \left( \psi \circ \chi \right) (X - Y) \: \dequ \mu^{(N_C, N_V)}_t(Y) \: \dequ \mu^{(N_C, N_V)}_t(X)\\
...
\begin{align}
    \begin{split}
        \frac{\partial \bar{\rho}_t^{\:N_V}(X)}{\partial t} - \nabla \cdot \Biggl[ \bar{\rho}_t^{\:N_V}(X) \left(\nabla_X E_A(X) + \nabla_X E_E(X) + \nabla_X E_I(X)\right) \\
        + \alpha_{\bar{O}} \bar{\rho}_t^{\:N_V}(X) \left(\nabla_{X} \left( \psi \circ \chi \right) * \bar{\rho}_t^{\:N_V}\right)\!(X)\Biggr] = 0.
    \end{split}
\end{align}
% The shape preserving gradients are given in the Equations~\eqref{eq:cGradArea}, \eqref{eq:cGradEdge}, \eqref{eq:cGradAngle}. %TODO: add bounce gradient reference.
