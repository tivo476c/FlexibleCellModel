...
\begin{align}
    \frac{\partial \bar{\rho}_t^{\:N_V}(\vec{x})}{\partial t} - \nabla \cdot (\bar{\rho}_t^{\:N_V}(\vec{x}) \nabla E(\vec{x})) = 0.
...
Furthermore, we assume that $\rho_t^{(N_C, N_V)}$ converges to the mean field density $\bar{\rho}_t^{\:N_V} \in \mathcal{P}(\R^{2 N_V})$, as $N_C \rightarrow \infty$.
Then, we compute
\begin{align*}
...
\begin{align}
    \frac{\partial \bar{\rho}_t^{\:N_V}(\vec{x})}{\partial t} - \nabla \cdot \left[\alpha_{\bar{O}} \bar{\rho}_t^{\:N_V}(\vec{x}) \left(\nabla_{\vec{x}} \left( \psi \circ \chi \right) * \bar{\rho}_t^{\:N_V}\right)\!(\vec{x}) \right] = 0.
