...
We now want to derive a temporal PDE for the time evolution of the density of a $2$-dimensional point particle system. 
...
\begin{align}
    \begin{split}
        ...
        &= - \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \nabla_{\vec{x}_i^{\: (s)}} \phi(\vec{x}_i^{\: (s)}(t)) \cdot \nabla_{\vec{x}_i^{\: (s)}} E(\vec{x}_i^{\: (s)}(t)) \\
...
When we consider higher-dimensional DF cells $C \in \mathbb{R}^{2N_V}$, however, the state space increases accordingly to $\mathbb{R}^{2N_V}$, making it much harder to visualise the density and its evolution over time.
...
\end{align}
For each energy $E$, we need a representation of the cell wise gradient $\nabla_C E(X)$ that we can plug into the mean field PDE from Equation~\eqref{equ:meanfieldPDE}. \\
...
\end{align*}
with $\bar{O}(\vec{C})$ defined in Equation~\eqref{energy:bounce}, with a constant scaling factor $\alpha_{\bar{O}} > 0$.
...
For the computation, we use the convolution notation $f * g$ for two functions $f,\, g:\, \R^{d} \to \R$, $d \in \N$ and according variables $\vec{x}, \vec{y} \in \R^d$ defined as
\begin{align*}
...
\end{align*}
This dynamics coincides with the deterministic drift of the DF SDE, given in Equation~\eqref{DF-deterministic-force}.
...
We use these results, and consider in the mean field limit ($N_C \rightarrow \infty$), to obtain  
\begin{align*}
    \int \phi(X) \,\frac{\partial}{\partial t} \left[\bar{\rho}_t^{\:N_V}(X)\right] \: \dequ X
