...
\begin{definition} \textbf{Bounce overlap energy} \label{energy:bounce}\\
    ...
    \end{align*}
    where we use the functions two function $\chi$ and $\psi$, defined as follows. \\
    The first function 
    \begin{align*}
...
\begin{proposition} \textbf{Bounce overlap gradient} \label{prop:bounceForce}\\
    The bounce overlap force from Definition~\ref{def:bounceForce} is the negative gradient of the bounce overlap energy from Definition~\ref{energy:bounce}, i.e. 
    \begin{align*}
        F^{(\bar{O})}_i(\vec{C}) = - \nabla_{C_i} \bar{O}(\vec{C}).
    \end{align*}

    Proof.\\
    We compute the gradient of the bounce overlap energy for cell $i$ as
    \begin{align*}
        \nabla_{C_i} \bar{O}(\vec{C}) 
        &= \nabla_{C_i} \left[ \frac{1}{N_C} \sum\limits_{m=1}^{N_C} \sum\limits_{j > m}^{N_C}  \left( \psi \circ \chi \right) (C_m - C_j) \right] \\
        &= \frac{1}{N_C} \sum\limits_{j \neq i}  \nabla_{C_i} \left[  \left( \psi \circ \chi \right) (C_i - C_j) \right] \\
        &= \frac{1}{N_C} \sum\limits_{j \neq i}  \psi'(\chi(C_i - C_j)) \nabla_{C_i} \chi (C_i - C_j). \\
    \end{align*}
    The first derivative of $\psi$ is given by
    \begin{align*}
        \psi'(d) 
        &= \frac{N_V}{2} \frac{\dequ}{\dequ d} \left[\mathbf{1}_{\{d < 2r\}} (2r - d)^2\right] \\
        &= \frac{N_V}{2} \Bigl( \underbrace{\frac{\dequ}{\dequ d} \left[\mathbf{1}_{\{d < 2r\}}\right] (d - 2r)^2}_{= 0} + \mathbf{1}_{\{d < 2r\}}\frac{\dequ}{\dequ d} \left[(d - 2r)^2\right] \Bigr)\\
        &= \tfrac{N_V}{2} \mathbf{1}_{\{d < 2r\}} 2(d - 2r) \underbrace{\frac{\dequ}{\dequ d} \left[(d - 2r)\right]}_{=1}\\
        &= N_V \mathbf{1}_{\{d < 2r\}} (d - 2r).\\
    \end{align*}
...
Fortunately, we can easily do so, as we just add a zero with the additional term, because 
%TODO: yeah compute properly ... problem: 0/0 in derivative? 
\begin{align*}
    \nabla_{C_i} \left( \psi \circ \chi \right) (C_i - C_i) 
    = \nabla_{C_i} \left( \psi \circ \chi \right)(0) 
    = 0,
\end{align*}
as $\left( \psi \circ \chi \right)(0)$ is independent of $C_i$. \\
...
\[
    \mu^{(N_C, N_V)} = \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \delta_{C_i(t)},
\]
for a given cell system $\vec{C}(t) = (C_1(t), \ldots, C_{N_C}(t))$ and assume convergence of the measure to a mean field density $\bar{\rho}_t^{\:N_V} \in \mathcal{P}(\R^{2 N_V})$, as $N_C \to \infty$.
Then, we compute
\begin{align*}
    ...
    &= - \frac{\alpha_{\bar{O}}}{{N_C}}  \int \sum\limits_{j = 1}^{N_C}  \nabla \phi(x) \cdot  \nabla_{C_i} \left( \psi \circ \chi \right) (x - C_j(t)) \: \dequ \mu^{(N_C, N_V)}_t(x)\\
    &= - \alpha_{\bar{O}}  \iint  \nabla \phi(x) \cdot  \nabla_{C_i} \left( \psi \circ \chi \right) (x - y) \: \dequ \mu^{(N_C, N_V)}_t(y) \: \dequ \mu^{(N_C, N_V)}_t(x)\\
    &\xrightarrow{N_C \to \infty} \: - \alpha_{\bar{O}}  \iint  \nabla \phi(x) \cdot  \nabla_{C_i} \left( \psi \circ \chi \right) (x - y) \, \bar{\rho}_t^{\:N_V}(y)\, \dequ y \: \bar{\rho}_t^{\:N_V}(x) \,\dequ x\\
    &= - \alpha_{\bar{O}}  \int  \nabla \phi(x) \cdot  \left(\nabla_{C_i} \left( \psi \circ \chi \right) * \bar{\rho}_t^{\:N_V}\right)\! (x)  \bar{\rho}_t^{\:N_V}(x) \:\dequ x \\
    &= \alpha_{\bar{O}}  \int  \phi(x) \nabla \cdot \left[ \left(\nabla_{C_i} \left( \psi \circ \chi \right) * \bar{\rho}_t^{\:N_V}\right)\!(x)  \bar{\rho}_t^{\:N_V}(x) \right] \: \dequ x \\
    &=  \int  \phi(x) \nabla \cdot \left[\alpha_{\bar{O}} \bar{\rho}_t^{\:N_V}(x) \left(\nabla_{C_i} \left( \psi \circ \chi \right) * \bar{\rho}_t^{\:N_V}\right)\!(x) \right] \: \dequ x. \\
\end{align*}
