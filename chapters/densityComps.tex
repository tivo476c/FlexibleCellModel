...
\begin{align*}
    ...
    &= \psi'(\chi(C_i - C_i)) \nabla_{C_i} \chi (C_i - C_i) \\
    &= \mathbf{1}_{\{\norm[\vec{x}_i - \vec{x}_i] < 2r\}} (\norm[\vec{x}_i - \vec{x}_i] - 2r)  \begin{pmatrix}
            I_2 \\ 
            \vdots \\ 
            I_2
        \end{pmatrix} \frac{\vec{x}_i - \vec{x}_i}{ \norm[\vec{x}_i - \vec{x}_j]}.
\end{align*}
...
\]
for a given cell system $\vec{C}(t) = (C_1(t), \ldots, C_{N_C}(t))$ and assume convergence to a mean field density $\bar{\rho}_t^{\:N_V} \in \mathcal{P}(\R^{2 N_V})$ as $N_C \to \infty$.
Then, we compute
\begin{align*}
    ...
    &= - \alpha_{\bar{O}}  \iint  \nabla \phi(x) \cdot  \nabla_{C_i} \left( \psi \circ \chi \right) (x - y) \: \dequ \mu^{(N_C, N_V)}_t(x)\dequ \mu^{(N_C, N_V)}_t(y)\\
    ...
    &=  \int  \phi(x) \nabla \cdot \left[\alpha_{\bar{O}} \left(\nabla_{C_i} \left( \psi \circ \chi \right) * \bar{\rho}_t^{\:N_V}\right)\! (x) \: \bar{\rho}_t^{\:N_V}(x) \right] \: \dequ x. \\
\end{align*}
