...
For a two-dimensional particle system, the mean-field density $\bar{\rho}_t$ is defined on the state space $\mathbb{R}^2$, corresponding to the positions of individual particles, as illustrated by the empirical measures in Figure~\ref{fig:muTransition}. 
When we consider higher-dimensional DF cells $C \in \mathbb{R}^{2N_V}$, however, the state space increases to $\mathbb{R}^{2N_V}$, making it much harder to visualise the density and its evolution over time.

\subsection{Low-dimensional needles example}
...
\begin{align}
    \begin{split}
        ...
        \end{pmatrix} 
        =  \dfrac{\alpha_{A}}{2} (A_{C} - A_d) M_A C.
    \end{split}
    \label{eq:cGradArea}
\end{align}
...
We define matrices for the prefactors: $D_E, D_{E-} \in \R^{2 N_V \times 2 N_V}$ as
\begin{align*}
...
\begin{align}
    \nabla_C E_E(C) = \alpha_{E} \left(  D_{E-} M_{E-} + D_{E} M_{E} \right) C.
    \label{eq:cGradEdge}
\end{align}
...
\end{align*}
The terms of form $\frac{1}{(E^j_C)^2}$ and  $\frac{1}{(E^{j+1}_C)^2}$ are summarised in $B_I, B_{I+} \in \R^{2N_V \times 2N_V}$ as, 
\begin{align*}
...
\begin{align}
    \nabla_C E_I(C) = \alpha_I \left(  - D_{I-} B_I M_{I-}  + D_I \left(  B_I M_{I-}  - B_{I+}  M_{I+} \right) + D_{I+} B_{I+} M_{I+}  \right) C.
    \label{eq:cGradAngle}
\end{align}
...
Let us use pure scaled bounce overlap energy, i.e.
\begin{align*}
    E_{\bar{O}}(\vec{C}) = \alpha_{\bar{O}} \bar{O}(\vec{C}) = \frac{\alpha_{\bar{O}}}{N_C} \sum\limits_{i=1}^{N_C} \sum\limits_{j > i}^{N_C}  \left( \psi \circ \chi \right) (C_i - C_j),
\end{align*}
with $\bar{O}(\vec{C})$ defined in Equation~\eqref{energy:bounce} and an additional constant scaling factor $\alpha_{\bar{O}} > 0$.
That results in the cell dynamic 
\begin{align*}
    \frac{\partial C_i}{\partial t} = - \nabla_{C_i} E_{\bar{O}}(\vec{C}) = - \frac{\alpha_{\bar{O}}}{N_C} \sum\limits_{j = 1}^{N_C}  \nabla_{C_i} \left( \psi \circ \chi \right) (C_i - C_j).
\end{align*}
...
\begin{align*}
    E_{\text{hard}}(\vec{C}) =  E_{\bar{O}}(\vec{C}) + \sum\limits_{k=1}^{N_C} \left(  E_A(C_k) + E_E(C_k) + E_I(C_k)\right).
\end{align*}
...
\begin{align*}
    ...
    &= - \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \nabla_{C_i} \phi(C_i(t)) \cdot \left[ \nabla_{C_i} E_{\bar{O}}(\vec{C}) + \nabla_{C_i}E_A(C_i) + \nabla_{C_i}E_E(C_i) + \nabla_{C_i} E_I(C_i) \right] \\
    &= - \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \nabla_{C_i} \phi(C_i(t)) \cdot \left[ \nabla_{C_i} E_{\bar{O}}(\vec{C}) \right] 
    - \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \nabla_{C_i} \phi(C_i(t)) \cdot \left[ \nabla_{C_i}E_A(C_i) \right]  \\
    &\quad - \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \nabla_{C_i} \phi(C_i(t)) \cdot \left[ \nabla_{C_i}E_E(C_i) \right] 
    - \frac{1}{N_C} \sum\limits_{i=1}^{N_C} \nabla_{C_i} \phi(C_i(t)) \cdot \left[ \nabla_{C_i} E_I(C_i) \right]. \\
\end{align*}
...
\begin{align}
    ...
    \end{split}
\end{align}
where the bounce overlap gradient is given in Equation~\eqref{cellgrad:bounce} and the shape preserving gradients are given in the Equations~\eqref{eq:cGradArea}, \eqref{eq:cGradEdge}, \eqref{eq:cGradAngle}.
