...
This appendix provides additional computations and intermediate analyses that were carried out during the course of this study but did not ultimately form part of the main body of the thesis. 
Their inclusion here aims to offer a more comprehensive account of the exploratory steps and supplementary considerations that informed the final results. 
Moreover, the author acknowledges the use of AI-based language tools, in particular chatbots~\cite{Openai2025, SAIA2024}, which were employed exclusively to improve wording and clarity.
All scientific content, including mathematical derivations, modelling arguments, and analytical reasoning, was produced and rigorously validated by the author. 

\subsection*{Computation of $\nabla_{\vec{v}_j} \cdot \nabla_{\vec{v}_j} A_2(C)$} 
We consider pure area energy 
\begin{align*}
...
We consider pure edge energy 
\begin{align*}
    E_2(C) =  \sum\limits_{j=1}^{N_V} \frac{1}{2} |E^j_{C} - E^{j}_d|^2, 
\end{align*}
defined at Equation~\eqref{eq:edgeEnergy} with $k=2$.
The gradient is given in Equation~\eqref{gradient:edge} as 
\begin{align*}
...
\end{align*}
\begin{align*}
    &= \frac{E_d^{j-1}}{(E^{j-1}_{C})^3} (\underbrace{(v_{j}^{x} - v_{j-1}^{x})^2 + (v_{j}^{y} - v_{j-1}^{y})^2}_{\displaystyle = (E^{j-1}_{C})^2}) + \dfrac{2(E^{j-1}_{C}- E_d^{j-1})}{E^{j-1}_{C}} \hspace*{1.3cm} \\
