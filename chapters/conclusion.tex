\section{Conclusion}
In this work, we developed a vertex based cell model: the DF model. 
It captures the behaviour of deformable cells with realistic mechanical interactions, including shape preservation, cell-cell overlap resolution, and stochastic motion. 
The first chapter provided a brief introduction to the DF cell model and its dynamics. 
We began by defining what a DF cell is and how such a cell is represented through its ordered sequence of vertices. 
...
Building on the initial vertex based formulation, first introduced in~\cite{Vogel2023}, we redefined the dynamics through suitably chosen area, edge, and interior angle energies
\begin{align*}
    A_k(C) &= \tfrac{1}{k} |A_{C} - A_d|^k, \quad
    E_k(C) &=  \sum\limits_{j=1}^{N_V} \tfrac{1}{k} |E^j_{C} - E^{j}_d|^k, \quad
...
\begin{align*}
\begin{split}
    \nabla_{\vec{v}_j^{\: i}} \hat{O}_k(\vec{C}) &= \sum\limits_{D \in \Omega_{C_i,C_m}} 
    |A_{D}|^{k-1} \Biggl(
...
\end{split}
\end{align*}
This force dissolves cell overlaps by moving overlap involved vertices out of the overlap, thereby changing the cell shape.
In order to be able to model the dynamic of the HSCM from Bruna and Chapman in~\cite{Bruna2012} more closely, we also implemented the bounce overlap force.
Unlike the deforming overlap force, the bounce overlap force does not change cell shapes, but it transports the whole cell away from overlapping cells.
The vertex gradient is given by
\begin{align*}
...
Altogether, the resulting deterministic part $\F^i_j$ from SDE~\eqref{SDE:DFvertexwise} can be written as 
\begin{align*}
    \F^i_j(\vec{C}(t)): (\R^{2N_V})^{N_C} \:&\longrightarrow\: \R^{2}, \\[0.5em]
    \F^i_j(\vec{C}(t)) 
    =\;  - \nabla_{\vec{v}^{\:i}_j} \bigl(&\alpha_{A}   A_2(C_i(t))  
    + \alpha_{E} E_2(C_i(t))  
    + \alpha_{I} I_2(C_i(t))  \\
    &+ (1-h)\alpha_{\hat{O}}  \hat{O}_{1}(\vec{C}(t)) 
    + h \alpha_{\bar{O}}\bar{O}(\vec{C}(t))\bigr).
\end{align*}
Definition~\ref{def:df-sde} also states the DF SDE in a cell wise view. \\
...
This behaviour confirms the intended interplay between the interaction forces and shape preserving forces, regulated by the hardness parameter. \\
In the final chapter, we derived the mean field description corresponding to the hard DF dynamics (hardness $h=1$). 
...
In this case, the resulting PDE for the mean field density $\bar{\rho}_t^{\:N_V}$ reads  
\begin{align*}
...
The gradient $\nabla_X \left( \psi \circ \chi \right)(X)$ is given in Equation~\eqref{grad:bounceConvolution}.
