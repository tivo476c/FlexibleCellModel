...
This is done by discretizing the domain into a uniform grid ofsubsquares. 
For eachsubsquare, we count how many cells fall within it across all simulations. 
The resulting counts are normalised by dividing by the total number of cells $N_C$, the number of simulations, and the area of asubsquare. 
...
After confirming that our implementation correctly reproduces the macroscopic dynamics of non-overlapping (hard) particles described in~\cite{Bruna2012}, we now generalise the model to account for cell deformability. \\ 
...
\[
    \alpha_{F} = 4 \pi \frac{\mathrm{area}_{F}}{\mathrm{perimeter}_{F}^{2}},
\]
where the scaling factor $4 \pi$ ensures that a perfect circle yields $\alpha_{\text{c}} = 1$. 
...
For a circle of radius $r>0$ we have $\text{area}_{\text{c}} = \pi r^2$ and $\text{perimeter}_\text{c} = 2\pi r$. 
Hence
\[
...
There, we can also see an indictation that as the number of vertices increases, the asphericity approaches $1$, consistent with the geometric interpretation that polygons with more vertices better approximate a circular shape.
...
The apshericity values below $0.7$ where reached in the $50$ simulations of the soft DF model, and can not be seen in Figure~\ref{fig:asp_charts}, since it was limited to the range $[0.7, 0.92]$ for better visual clarity.
