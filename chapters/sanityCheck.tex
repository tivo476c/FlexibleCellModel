...
%%% TODO: specify which julia packages where used, also cite them 
- All codes were implemented in Julia~\cite{Julia2017} \\
- We used Distributions.jl~\cite{Distributions2021} and its function `MvNormal()' to generate the normal distributed points \\
- The Julia package~\cite{DifferentialEquations2017} `DifferentialEquations.jl' was used to solve the SDEs with its inbuilt Euler Maruyama scheme with static time step size. \\
- all self made plots in this thesis are created with Plots.jl~\cite{Plots2023}. \\
- parallelisation of the monte carlo simulations was done with Julia built in `Distributed' package~\cite{Distributed2023}. \\
...
% This term encodes the excluded-volume interaction between finite-sized particles, which biases motion away from regions of high density.
% Consequently, the effective collective diffusion increases with both particle number \(N_C\) and particle size \(\epsilon\).
% Locally, diffusion is faster where \(\rho\) is large, since crowding produces a stronger flux toward less populated regions.
% We can rewrite the marginal equation~\eqref{equ:marginalHSCM} to explicitly show this enhanced diffusion effect as follows: \\
% \begin{align*}
...
%     ,
% \end{align*}
% where we used the product rule for gradients in the second step. \\
% We interpret \( D(\epsilon, N_C, \rho) = 1 + \pi (N_C - 1)\epsilon^2 \rho \) as an effective diffusion coefficient that depends on the cell diameter \(\epsilon\), the number of cells \(N_C\), and the local density \(\rho\). 
% When setting \(\epsilon = 0\), we retrieve the diffusion equation for point particles in Equation~\eqref{equ:marginalPP}, i.e., \( D(0, N_C, \rho) = 1 \).
% For $\epsilon > 0$ (and $N_C > 1$), the diffusion coefficient \( D(\epsilon, N_C, \rho) > 1 \) increases with both \(\epsilon\) and \(N_C\), leading to an enhanced diffusion rate compared to point particles.
% This reformulation clearly illustrates how the excluded-volume interactions in the hard sphere cell model lead to faster diffusion relative to the point particle model. \\ 
...
% TODO: find a paper explaining monte carlo sims 
