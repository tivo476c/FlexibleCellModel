...
The point particle model and HSCM from~\cite{Bruna2012} were already introduced in Section~\ref{intro}. 
...
\]
By the Central Limit Theorem, the empirical heatmaps converge to their expectation, which coincides with the first marginal distribution of the system, as both the number of i.i.d. simulations increases and the spatial discretisation of $\Omega$ is refined.
This procedure yields a smooth and consistent estimate of the empirical cell density, enabling a direct comparison with the corresponding solutions of the continuum diffusion equations.
...
This modification leads to an extended stochastic description in which the deforming overlap force $F_i^{(\hat{O})}(\vec{C})$, defined in Proposition~\ref{force:deformingOverlap}, supplements the purely repulsive bounce overlap force $F_i^{(\bar{O})}(\vec{C})$ (Proposition~\ref{prop:bounceForce}), whenever we use a hardness $h < 1$.
